import { useTranslation } from 'react-i18next';

const STEPS = [
  { icon: '🔍', key: 'step1' },
  { icon: '🚗', key: 'step2' },
  { icon: '✅', key: 'step3' },
  { icon: '✈️', key: 'step4' },
] as const;

export function HowItWorks() {
  const { t } = useTranslation();

  return (
    <section id="how-it-works" className="bg-white py-20 px-6">
      <div className="mx-auto max-w-7xl">
        <div className="text-center mb-14">
          <span className="text-emerald-600 text-sm font-bold uppercase tracking-widest">
            {t('home.howItWorks.badge')}
          </span>
          <h2 className="text-3xl md:text-4xl font-extrabold text-slate-900 mt-2 mb-4 tracking-tight">
            {t('home.howItWorks.title')}
          </h2>
          <p className="text-slate-500 max-w-xl mx-auto leading-relaxed">
            {t('home.howItWorks.desc')}
          </p>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 relative">
          {/* bağlayıcı çizgi */}
          <div className="hidden lg:block absolute top-14 left-[12.5%] right-[12.5%] h-0.5 bg-gradient-to-r from-transparent via-emerald-200 to-transparent" />

          {STEPS.map((s, i) => (
            <div key={s.key} className="relative flex flex-col items-center text-center group">
              <div className="relative z-10 w-16 h-16 bg-emerald-50 group-hover:bg-emerald-100 border-2 border-emerald-100 group-hover:border-emerald-300 rounded-2xl flex items-center justify-center mb-5 transition-all duration-200 shadow-sm text-2xl">
                {s.icon}
                <span className="absolute -top-2 -right-2 w-6 h-6 bg-emerald-500 text-white text-xs font-bold rounded-full flex items-center justify-center shadow-md">
                  {i + 1}
                </span>
              </div>
              <h3 className="text-slate-900 font-bold text-lg mb-2">
                {t(`home.howItWorks.${s.key}.title`)}
              </h3>
              <p className="text-slate-500 text-sm leading-relaxed">
                {t(`home.howItWorks.${s.key}.desc`)}
              </p>
            </div>
          ))}
        </div>
      </div>
    </section>
  );
}
